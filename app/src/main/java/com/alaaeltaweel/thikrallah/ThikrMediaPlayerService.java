...
public class ThikrMediaPlayerService extends Service implements OnCompletionListener,
    ...
    // ✅ لازم نحتفظ بنفس كائن طلب التركيز الصوتي ده عشان نقدر نلغيه صح بعدين - إلغاء بطريقة
    // قديمة مش مرتبطة بنفس الطلب كان بيخلي الأندرويد مايرجّعش التركيز فعليًا للتطبيق التاني
    private AudioFocusRequest mFocusRequest;

    // ✅ علامة بسيطة: الأذان ده لازم يبدأ مكتوم لأن فيه مكالمة (عادية أو نت) كانت شغالة
    // بالفعل وقت ما بدأ - بنخليه يكمل لحد ما يخلص بدل ما يوقف أو يشتغل بصوت عالي فوق المكالمة
    private boolean athanShouldStartMuted = false;
    ...
    public void onDestroy() {
        ...
        // ✅ إلغاء التركيز بنفس طريقة الطلب بالظبط - وإلا الأندرويد مايرجّعش التركيز فعليًا
        // للتطبيق التاني (زي انستا/يوتيوب)، وصوته يفضل مكتوم من غير رجوع
    ...
    public void onAudioFocusChange(int focusChange) {
        ...
        switch (focusChange) {
            ...
            case AudioManager.AUDIOFOCUS_LOSS:
                ...
                // ✅ لو ده أذان وفقد التركيز (زي مكالمة بدأت لحظة ما كان شغال)، نكتمه ونكمّله
                // لحد ما يخلص بدل ما نوقفه - بدل ما يتقطع فجأة في نص الأذان
                if (this.getThikrType() != null && this.getThikrType().contains(MainActivity.DATA_TYPE_ATHAN)) {

                    Timber.d("lost focus but this is athan - muting and continuing instead of stopping");

                    athanShouldStartMuted = true;

                    if (player != null) { try { player.setVolume(0f, 0f); } catch (Exception ignored) {} }
            ...
            case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                ...
                // ✅ نفس فكرة AUDIOFOCUS_LOSS - الأذان يكتم ويكمل بدل ما يتوقف مؤقتًا
                // وميرجعش تاني (استرجاع التركيز مش مضمون)
                if (this.getThikrType() != null && this.getThikrType().contains(MainActivity.DATA_TYPE_ATHAN)) {

                    Timber.d("transient loss but this is athan - muting and continuing instead of pausing");
    ...
    private void setVolume() {
        ...
        if (this.getThikrType().contains(MainActivity.DATA_TYPE_ATHAN)) {
            ...
            // ✅ لو الأذان بدأ مكتوم بسبب مكالمة شغالة، يفضل مكتوم - مايرجعش يتصاعد لوحده
            if (athanShouldStartMuted) {
            ...
            if (isGradual) {
                ...
                player.setVolume(getAthanMaxVolumeFloat(), getAthanMaxVolumeFloat());
    ...
    // ✅ فحص موحد للمكالمة (عادية أو نت) - بنعمله قبل أي محاولة لمس نظام الصوت خالص،
    // عشان مانلمسش تركيز الصوت أصلاً وقت المكالمة (بدل ما نطلبه ونلغيه على طول)
    private boolean isPhoneOrVoipCallActive() {
        ...
        // نظام أندرويد الرسمي للمكالمات - فحص أدق وأقل عرضة للغلط من وضع الصوت الخام بس
        try {
        ...
        try {
            ...
            AudioManager amCheck = (AudioManager) getSystemService(Context.AUDIO_SERVICE);

            if (amCheck != null && amCheck.getMode() == AudioManager.MODE_IN_COMMUNICATION) {
    ...
    private void startPlayerIfAllowed() {
        ...
        boolean isAthanTypeCheck = this.getThikrType() != null && this.getThikrType().contains(MainActivity.DATA_TYPE_ATHAN);

        boolean isGeneralThikrTypeCheck = this.getThikrType() != null && this.getThikrType().equalsIgnoreCase(MainActivity.DATA_TYPE_GENERAL_THIKR);

        boolean callActiveCheck = isPhoneOrVoipCallActive();

        // ✅ الذكر العام مايحاولش ياخد تركيز صوت خالص لو فيه مكالمة شغالة - بنقف هنا قبل
        // حتى ما نوصل لسطر طلب التركيز
        if (isGeneralThikrTypeCheck && callActiveCheck) {

            Timber.d("general thikr - call active, skipping without touching audio focus at all");
        ...
        // ✅ الأذان: لو فيه مكالمة شغالة بالفعل، لازم يبدأ مكتوم - يكمل لحد ما يخلص من غير
        // ما يوقف المكالمة ولا يشتغل بصوت عالي فوقها
        athanShouldStartMuted = isAthanTypeCheck && callActiveCheck;
        ...
        if (ret == AudioManager.AUDIOFOCUS_REQUEST_GRANTED) {
            ...
            if (athanShouldStartMuted) {
                ...
                Timber.d("athan started muted because a call is already active");
    ...
    private void incrementVolume() {
        ...
        // ✅ سقف الصوت بقى حسب إعداد مستوى صوت الأذان، مش 1 (أقصى صوت) دايمًا
        float athanCap = getAthanMaxVolumeFloat();

        if (fVolume < FLOAT_VOLUME_MIN)

            fVolume = FLOAT_VOLUME_MIN;

        else if (fVolume > athanCap)
