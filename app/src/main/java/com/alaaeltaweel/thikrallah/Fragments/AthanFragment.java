...
// ── ربط views التنبيه قبل الأذان ──
        ...
        // ✅ قسم الإعدادات العامة للأذان - فتح وقفل + تحميل وحفظ القيم
        View generalSettingsHeader = view.findViewById(R.id.general_settings_header);
        View generalSettingsContent = view.findViewById(R.id.general_settings_content);
        TextView generalSettingsArrow = view.findViewById(R.id.general_settings_arrow);

        generalSettingsHeader.setOnClickListener(v -> {
            boolean isOpen = generalSettingsContent.getVisibility() == View.VISIBLE;
            generalSettingsContent.setVisibility(isOpen ? View.GONE : View.VISIBLE);
            generalSettingsArrow.setText(isOpen ? "⌄" : "⌃");
        });

        // 1) الإشعار الدائم
        Switch gsForegroundTimer = view.findViewById(R.id.gs_foreground_timer_switch);
        gsForegroundTimer.setChecked(mPrefs.getBoolean("foreground_athan_timer", true));
        gsForegroundTimer.setOnCheckedChangeListener((btn, checked) ->
                mPrefs.edit().putBoolean("foreground_athan_timer", checked).apply());

        // 2) تدرّج الصوت
        Switch gsGradualVolume = view.findViewById(R.id.gs_gradual_volume_switch);
        gsGradualVolume.setChecked(mPrefs.getBoolean("gradual_volume", true));
        gsGradualVolume.setOnCheckedChangeListener((btn, checked) ->
                mPrefs.edit().putBoolean("gradual_volume", checked).apply());

        // 3) الدعاء بعد الأذان
        Switch gsDua = view.findViewById(R.id.gs_dua_switch);
        gsDua.setChecked(mPrefs.getBoolean("isDuaAfterAthan", false));
        gsDua.setOnCheckedChangeListener((btn, checked) ->
                mPrefs.edit().putBoolean("isDuaAfterAthan", checked).apply());

        // 4) الوضع الصامت وقت الصلاة
        Switch gsSilentMode = view.findViewById(R.id.gs_silent_mode_switch);
        android.widget.EditText gsSilentDuration = view.findViewById(R.id.gs_silent_mode_duration);
        gsSilentMode.setChecked(mPrefs.getBoolean("isSilentModeDuringPrayer", false));
        gsSilentDuration.setText(String.valueOf(mPrefs.getInt("silentModeDurationMinutes", 15)));
        gsSilentMode.setOnCheckedChangeListener((btn, checked) ->
                mPrefs.edit().putBoolean("isSilentModeDuringPrayer", checked).apply());
        gsSilentDuration.setOnFocusChangeListener((v, hasFocus) -> {
            if (!hasFocus) {
                try {
        ...
        // 5) طريقة حساب المواقيت (3 قوائم منسدلة)
        setupGeneralSpinner(view, R.id.gs_calc_method_spinner, R.array.athan_calculation_method_values, "calc_method", "4");
        setupGeneralSpinner(view, R.id.gs_asr_calc_method_spinner, R.array.asr_calculation_method_values, "asr_calc_method", "0");
        setupGeneralSpinner(view, R.id.gs_adjustment_method_spinner, R.array.athan_adjustment_methods_values, "adjustment_method", "0");

        // 6) تعديل التاريخ الهجري
        setupGeneralSpinner(view, R.id.gs_hijri_offset_spinner, R.array.hijri_offset_values, "hijri_offset", "0");

        return view;
    }

    // ✅ دالة مساعدة عامة لكل القوائم المنسدلة في قسم الإعدادات العامة
    private void setupGeneralSpinner(View parentView, int spinnerId, int valuesArrayResId, String prefKey, String defaultValue) {
        android.widget.Spinner spinner = parentView.findViewById(spinnerId);
        String[] values = getResources().getStringArray(valuesArrayResId);
        String currentValue = mPrefs.getString(prefKey, defaultValue);
        int selectedIndex = 0;
        ...
        spinner.setOnItemSelectedListener(new android.widget.AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(android.widget.AdapterView<?> parent, View v, int position, long id) {
                mPrefs.edit().putString(prefKey, values[position]).apply();
            }
            @Override
